...
compare_model = gen.load('glove-twitter-25')
...
class WebGraph():
    ...
    def search(self, pi, query='', max_results=10):
        ...
        for i in range(n):
            ...
            S = vectors.most_similar(terms)
            for thing in S:
                n =  # number of times word appears in document
                word_similarity =   # similarity score of word
                rank_score+= n * word_similarity**p
            '''
...
def url_satisfies_query(url, query):
    ...
    # task 1
    for x in others:
        similar_to_query = compare_model.most_similar(x, topn=5)
