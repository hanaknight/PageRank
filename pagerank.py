...
class WebGraph():
    ...
    def power_method(self, v=None, x0=None, alpha=0.85, max_iterations=1000, epsilon=1e-6):
        ...
        with torch.no_grad():
            ...
            # main loop: implementing equation 5.1
            # will need to run until x(k) - x^(k-1) <= epsilon
            # sparse matrix multiplication: first var sparse, second var dense
            # flip parameters in eq 5.1 -- first param is dense, second is sparse
            ...
            for i in range(n):
                if nondangle_nodes[i] == 0:
                    a[i] = 1

            # need to calculate x^t * a
            for k in range(0,max_iterations):
                x_latest = x  # store the latest value of x 
                alphax = alpha * x_latest.t()   # constant * size [1xn] = size [1xn] 
                lhs_term = torch.sparse.mm(self.P.t(), alpha.t()).t()  
                # ( [nxn]^transpose matrix_multiplication [1xn]^transpose )^ all transposed

                rhs_subterm = alphax * a + (1-alpha) # [1xn] matrix_multi [nx1] = [1xn] vector
                rhs_term = rhs_subterm * v.t()  # then multiply it with the v personalization vector
                x = (lhs_term + rhs_term).t() # new x variable 

                if torch.norm(x - x_latest) < epsilon:
                    break 
